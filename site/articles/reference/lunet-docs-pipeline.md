...
API docs are generated by Lunet `api.dotnet` from the library projects listed in [API Coverage Index](api-coverage-index). That set now includes the `Svg.Editor.*` packages in addition to the runtime, Avalonia, and generator-facing assemblies.
